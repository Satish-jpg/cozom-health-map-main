...
class BodyPartWidget(QWidget):
    ...
    def symptom_remove(self, symptom):
        # Removing the symptom from the list
        self.part.mSymptoms = [ x for x in self.part.mSymptoms if x.name.strip() != symptom.name.strip() ]
        self.method()

    # Added a user-friendly method to update the UI title
    def update_ui_title(self):
        self.setWindowTitle("Cozom - Body Part Details")
