        ...
        with open(os.sep.join((conditionPath, conditionsFilePath))) as conditionFile:
            ...
            for condition in conditions['data']['conditions']:
                if not condition is None:
                    # WebMD URL को Cozom URL से बदलें
                    updated_url = condition['curl'].replace("www.WebMD.com", "www.cozom.in")
                    symptom.add_condition(Condition(id=condition['id'], name=condition['name'], about=updated_url))
